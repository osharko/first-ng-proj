...
export class ZipComponent {
  ...
  public scelte = [
    ...
    { name: "Auto", checked: false, folder: "supercar", elements: [{ file: "Lambo.jpg", path: "https://www.autotoday.it/wp-content/uploads/2019/05/HURACAN-EVO.jpg", }, { file: "Koenigsegg.jpg", path: "https://sm.askmen.com/t/askmen_me/article/2/2015-frank/2015-frankfurt-auto-show-reveals_j7g5.1200.jpg" }] },
  ];

  public get IsEnabled(): boolean { return this.scelte.some(scelta => scelta.checked); }

  public get AllChecked(): boolean { return this.scelte.every(scelta => scelta.checked); }
  public get SomeChecked(): boolean { return this.scelte.some(scelta => scelta.checked) && this.scelte.some(scelta => !scelta.checked); }

  public constructor(private toolbarService: ToolbarService) {
    this.toolbarService.title = "Zipper";
  }

  public selectAll() {
    const checked = this.AllChecked;
    this.scelte.forEach(scelta => scelta.checked = !checked);
  }
  ...
  public download() {
    ...
    this.scelte
      .filter(scelta => scelta.checked)
      .forEach(scelta => {
        const currentFolder = zip.folder(scelta.folder);
        scelta.elements.forEach(element => currentFolder?.file(element.file, element.path));
      });
